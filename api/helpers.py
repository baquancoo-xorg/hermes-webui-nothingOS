...
def build_profile_cookie(name: str) -> str:
    ...
    # Without Max-Age this is a session cookie: closing the browser (or the
    # mobile OS evicting the PWA) drops it, and the next load falls back to
    # the process-global default profile even though the user had selected
    # another profile. Persist for a year so the selection survives restarts.
    cookie[cookie_name]['max-age'] = str(365 * 24 * 3600)
