...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/dash/cost":
        # Dashboard cost Opus thật từ claude -p jsonl (read-only). Auth như route khác.
    ...
    if parsed.path == "/api/dash/roster":
        # Dashboard team roster — map board (kanban_board) -> profiles qua registry.json
        # (read-only). Nguon chan ly thay cho doan prefix ten profile. Auth nhu /api/dash/cost.
        from api.auth import is_auth_enabled, parse_cookie, verify_session
        if is_auth_enabled():
            cv = parse_cookie(handler)
            if not (cv and verify_session(cv)):
    ...
    if parsed.path == "/api/ceo-reports":
        from api.profiles import cron_profile_context

        with cron_profile_context():
            return _handle_ceo_reports(handler, parsed)

    if parsed.path == "/api/agent-report":
        return _handle_agent_report(handler, parsed)
...
def _handle_ceo_reports(handler, parsed):
    """Return recent local CEO/progress report cron outputs for the WebUI.

    This is a read-only delivery surface for browser sessions where cron
    ``deliver=origin`` cannot resolve a chat target. It intentionally reads only
    scheduler output markdown files under the active profile's cron output
    directory and applies the shared redactor before returning content.
    """
    from cron.jobs import OUTPUT_DIR as CRON_OUT, list_jobs
    ...
    try:
        ...
        limit = 20
    include_all = str(qs.get("all", [""])[0]).lower() in ("1", "true", "yes")
    keywords = ("ceo", "quickmagic", "quick-magic", "product-ai", "product ai", "kanban")
    ...
    files = []
    try:
        job_dirs = CRON_OUT.iterdir() if CRON_OUT.exists() else []
        for job_dir in job_dirs:
            ...
            searchable = " ".join(
                str(job.get(k) or "") for k in ("name", "prompt", "script")
            ).lower()
            if not include_all and not any(k in searchable for k in keywords):
                continue
            for f in job_dir.glob("*.md"):
                try:
                    st = f.stat()
                    files.append((st.st_mtime, f, job_id, name, job))
                except OSError:
                    logger.debug("Failed to stat CEO report file %s", f)
    except Exception as exc:
        logger.debug("Failed to enumerate CEO reports in %s: %s", CRON_OUT, exc)

    reports = []
    for _mtime, f, job_id, name, job in sorted(files, key=lambda item: item[0], reverse=True)[:limit]:
        try:
            raw = f.read_text(encoding="utf-8", errors="replace")
            content = _redact_text(_cron_output_content_window(raw, limit=20000))
            snippet = _redact_text(_cron_output_snippet(raw, limit=700))
            reports.append({
                "job_id": job_id,
                "job_name": _redact_text(str(name)),
                "filename": f.name,
                "modified": _mtime,
                "size": f.stat().st_size,
                "status": job.get("last_status") if isinstance(job, dict) else None,
                "schedule": job.get("schedule") if isinstance(job, dict) else None,
                "content": content,
                "snippet": snippet,
            })
        except Exception:
            logger.debug("Failed to read CEO report file %s", f)
    return j(handler, {"reports": reports, "count": len(reports), "output_dir": str(CRON_OUT)})


def _candidate_agent_report_scripts(profile_home: Path) -> list[Path]:
    """Return profile-local reporter scripts in preference order."""
    scripts_dir = profile_home / "scripts"
    if not scripts_dir.is_dir():
        return []
    names = [
        "quickmagic_progress_5m.py",
        "product_ai_uiux_progress_reporter.py",
    ]
    seen: set[Path] = set()
    result: list[Path] = []
    for name in names:
        ...
        if p.is_file() and p not in seen:
            result.append(p)
            seen.add(p)
    for pattern in ("*progress*report*.py", "*progress_5m.py", "*report*.py"):
...
def _handle_agent_report(handler, parsed):
    """Return the current live report for the active WebUI profile.

    This endpoint is read-only. It executes only reporter scripts under the
    active profile's own scripts/ directory, with a short timeout, and redacts
    output before returning it to the browser. If no live script is available,
    it falls back only to cron output owned by the same active profile. Cross-
    profile cron output (for example QuickMagic watchdogs while viewing
    KAIPAY-CONTENT) must never be shown as the current agent report.
    """
    ...
    scripts = _candidate_agent_report_scripts(profile_home)

    env = os.environ.copy()
    env["HERMES_HOME"] = str(profile_home)
    if profile_home.parent.name == "profiles":
        env.setdefault("HERMES_BASE_HOME", str(profile_home.parent.parent))
    ...
    for script in scripts:
        try:
            ...
            return bad(handler, f"Report script timed out: {script.name}", status=504)
        except Exception as exc:
            logger.debug("Failed to run agent report script %s: %s", script, exc)
            continue
        content = _redact_text((proc.stdout or "").strip())
        err = _redact_text((proc.stderr or "").strip())
        ...
        if proc.returncode != 0:
            return bad(handler, f"Report script failed ({script.name}): {err or 'no stderr'}", status=500)
    ...
    try:
        ...
        jobs = []
    jobs_by_id = {str(job.get("id") or ""): job for job in jobs if isinstance(job, dict) and job.get("id")}
    active_profile = str(profile_name or "default")
    candidates: list[tuple[float, Path, str, dict]] = []
    if CRON_OUT.exists():
        for job_dir in CRON_OUT.iterdir():
            if not job_dir.is_dir():
                continue
            job = jobs_by_id.get(job_dir.name, {})
            job_profile = str(job.get("profile") or "default") if isinstance(job, dict) else "default"
            # Profile isolation: never show another profile's cron output as
            # the current profile's live agent report.
            if job_profile != active_profile:
                continue
            searchable = " ".join(str(job.get(k) or "") for k in ("name", "prompt", "script")).lower()
            if not any(k in searchable for k in ("ceo", "progress", "kanban", "kaipay", "content")):
                continue
            for f in job_dir.glob("*.md"):
                try:
                    raw_preview = f.read_text(encoding="utf-8", errors="replace")
                    # Script-only watchdogs write audit stubs for empty stdout;
                    # they are intentionally silent and should not become a
                    # visible current-agent report.
                    if "Status:** silent (empty output)" in raw_preview or "Status: silent (empty output)" in raw_preview:
                        continue
                    if not _cron_output_content_window(raw_preview, limit=20000).strip():
                        continue
                    candidates.append((f.stat().st_mtime, f, job_dir.name, job))
                except OSError:
                    continue
                except Exception:
                    logger.debug("Failed to inspect cron output candidate %s", f)
                    continue
    if candidates:
        mtime, f, job_id, job = sorted(candidates, key=lambda item: item[0], reverse=True)[0]
        raw = f.read_text(encoding="utf-8", errors="replace")
        return j(handler, {
            "ok": True,
            "source": "cron-output",
            "profile": profile_name,
            "job_id": job_id,
            "job_name": _redact_text(str(job.get("name") or job_id)),
            "filename": f.name,
            "content": _redact_text(_cron_output_content_window(raw, limit=20000)),
            "updated": mtime,
        })
    ...
    return j(handler, {
        ...
        "source": "none",
        "profile": profile_name,
        "content": "No live report script or cron output found for this profile yet.",
        "updated": time.time(),
    })
...
def _handle_cron_recent(handler, parsed):
    ...
    try:
        ...
        def _latest_output_is_silent(job_id: str) -> bool:
            try:
                out_dir = CRON_OUT / str(job_id)
                ...
                return "Status:** silent (empty output)" in raw or "Status: silent (empty output)" in raw
            except Exception:
                return False
        ...
        completions = []
        for job in jobs:
            job_profile = str(job.get("profile") or "default")
            if job_profile != active_profile:
                continue
            ...
            if isinstance(last_run, str):
                ...
                ts = float(last_run)
            if ts > since:
                if _latest_output_is_silent(str(job.get("id", ""))):
                    continue
                completions.append(
                    {
                        "job_id": job.get("id", ""),
