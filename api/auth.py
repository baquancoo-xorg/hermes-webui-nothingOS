...
def check_auth(handler, parsed) -> bool:
    ...
    if (parsed.path in PUBLIC_PATHS or parsed.path in ('/', '/index.html')
            or parsed.path.startswith('/static/') or parsed.path.startswith('/session/')
            or parsed.path == '/v2' or parsed.path.startswith('/v2/')):
        return True
