...
// Background tabs must not keep polling: every open Hermes tab used to run
// its own 5s/30s/60s timers even while hidden, so N tabs multiplied both the
// /api/sessions load on the server and client-side DOM rebuild work. Timers
// now stop while document.hidden and resume (with one immediate refresh) on
// the next visibilitychange back to visible.
let _streamingPollWantedWhileHidden = false;

function _pollingPausedForHiddenTab(){
  return typeof document !== 'undefined' && document.hidden;
}

function startStreamingPoll(){
  _streamingPollWantedWhileHidden = true;
  if(_streamingPollTimer) return;
  if(_pollingPausedForHiddenTab()) return;
...
function stopStreamingPoll(){
  _streamingPollWantedWhileHidden = false;
  if(!_streamingPollTimer) return;
...
function _pausePollingTimersWhileHidden(){
  if(_streamingPollTimer){
    clearInterval(_streamingPollTimer);
    _streamingPollTimer = null;
    _streamingPollWantedWhileHidden = true;
  }
  if(_sessionTimeRefreshTimer){
    clearInterval(_sessionTimeRefreshTimer);
    _sessionTimeRefreshTimer = null;
  }
  if(_activeSessionExternalRefreshTimer){
    clearInterval(_activeSessionExternalRefreshTimer);
    _activeSessionExternalRefreshTimer = null;
  }
}

function _resumePollingTimersOnVisible(){
  ensureSessionTimeRefreshPoll();
  ensureActiveSessionExternalRefreshPoll();
  if(_streamingPollWantedWhileHidden) startStreamingPoll();
}

if(typeof document !== 'undefined' && !document._hermesPollingVisibilityHook){
  document.addEventListener('visibilitychange', () => {
    if(document.hidden) _pausePollingTimersWhileHidden();
    else _resumePollingTimersOnVisible();
  });
  document._hermesPollingVisibilityHook = true;
}
