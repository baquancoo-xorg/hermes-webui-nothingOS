...
const _SKINS=[
  ...
  {name:'Nothing',  value:'nothing', colors:['#f7f7f2','#0b0b0b','#ff0033']},
...
function _normalizeAppearance(theme,skin){
  ...
  const nextSkin=_VALID_SKINS.has(rawSkin)?rawSkin:(legacy?legacy.skin:'nothing');
...
(async()=>{
  ...
  if(saved){
    try{
      ...
      // Realign the client-active profile with the restored session's profile.
      // The hermes_profile cookie is the routing authority, but it can be
      // missing (dropped session cookie after a browser/PWA restart) or point
      // at another profile. Before this realign, a reload of /session/<id>
      // flipped the agent selector and the sidebar's profile scope back to
      // 'default' even though the open session belongs to another agent.
      try{
        const _normProfile=p=>{const v=String(p||'').trim();return(!v||v==='default')?'default':v;};
        const _sessProfile=_normProfile(S.session&&S.session.profile);
        if(S.session&&_sessProfile!==_normProfile(S.activeProfile)){
          const _sw=await api('/api/profile/switch',{method:'POST',body:JSON.stringify({name:_sessProfile})});
          S.activeProfile=(_sw&&_sw.active)||_sessProfile;
          S.activeProfileIsDefault=_sw&&('is_default' in _sw)?!!_sw.is_default:_normProfile(S.activeProfile)==='default';
          const _bootProfileLabel=$('profileChipLabel');
          if(_bootProfileLabel)_bootProfileLabel.textContent=S.activeProfile||'default';
          if(typeof applyBotName==='function')applyBotName();
        }
      }catch(_){/* keep the current active profile when the switch fails */}
  ...
  // Restore the active panel after boot finishes (every early-return path
  // included). switchPanel() persists '#panel=<name>' in the URL hash; without
  // this restore a reload always lands back on the chat tab. Hidden tabs are
  // skipped — the flash-prevention script in index.html already forces those
  // back to chat.
  try{
    const _m=/(?:^|[#&])panel=([^&]+)/.exec(location.hash||'');
    const panel=_m?decodeURIComponent(_m[1]):'';
    if(panel&&panel!=='chat'&&typeof switchPanel==='function'){
      const btn=document.querySelector(`[data-panel="${panel}"]`);
      if(btn&&!btn.classList.contains('nav-tab-hidden')) void switchPanel(panel);
    }
  }catch(_){}
});
